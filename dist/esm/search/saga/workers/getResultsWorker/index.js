...
import { addIgnoreOperators, getIgnoreOperators, getHotelsIDsFromPrices, generateNextPrices, getHotelsEntitiesMap, getOffersEntitiesMap, getUnusedPrices } from './helpers';
import { GUARANTEED_RESULT_STEP } from './constants';
export function getResultsWorker(_ref) {
  let {
    payload: queryID
  } = _ref;
  return function* () {
    const query = yield select(state => getQuery(state, {
      queryID
    }));
    ...
    try {
      do {
        ...
        memory.incTotal(total);
        {
          ...
          const freshSelectedOperators = yield select(state => getQuery(state, {
            queryID
          }).get(QUERY_PARAMS.SELECTED_OPERATORS).toJS());
          const nextPrices = generateNextPrices([...pricesWithoutStable, ...unusedPrices, ...usedPrices], offersHub, query.get(QUERY_PARAMS.CURRENCY), query.get(QUERY_PARAMS.SORT), hotelsHub, freshSelectedOperators);
        ...
        !R.isEmpty(ignoreOperators) && addIgnoreOperators(otpsukQuery, ignoreOperators);
      ...
      var _error$response;
      yield put(searchActions.failSearch(queryID, error, {
