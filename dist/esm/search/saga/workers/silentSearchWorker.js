...
import { generateNextPrices, getHotelsEntitiesMap, getHotelsIDsFromPrices, getOffersEntitiesMap, getTotalBySelectedOperators, getUnusedPrices } from "./getResultsWorker/helpers";
export function silentSearchWorker(_ref) {
  let {
    payload: queryID
  } = _ref;
  return function* () {
    ...
    const nextPrices = generateNextPrices(prices, offersHub, query.get(QUERY_PARAMS.CURRENCY), query.get(QUERY_PARAMS.SORT), hotelsHub, selectedOperators);
    ...
    yield put(searchActions.processSearch(queryID, {
      page: 1,
      prices: nextPrices,
      total: getTotalBySelectedOperators({
        offersHub,
        prices,
        selectedOperators
      })
