...
import { getResultsWorker, getPriceChartWorker, getAvailableDatesWorker, silentSearchWorker, warmupSearchWorker } from './workers';
export const searchWatchers = Object.freeze({
  *runSearchWatcher() {
    yield takeEvery(actions.getResults, function* (actionArgs) {
      ...
      const [cancelledTask] = yield race([take(action => R.and(R.equals(action.type, actions.cancelledSearch.toString()), R.equals(action.payload, queryId))), take(action => R.and(R.equals(action.type, actions.finishSearch.toString()), R.equals(R.prop('queryId', action.payload), queryId))), take(action => R.and(R.equals(action.type, actions.failSearch.toString()), R.equals(action.payload, queryId)))]);
  ...
  *clearSearchWorker() {
    yield takeEvery(actions.clearSearch, _ref => {
      let {
        payload: queryID
      } = _ref;
