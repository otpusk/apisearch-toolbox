...
// Instruments
import { binaryCompiler, rangeCompiler, datesCompiler, numberCompiler, toStringCompiler, immutableArrayCompiler } from './compilers';
import { rangeParser, datesParser, binaryParser, createImmutableArrayParser, createImmutableNumbersArrayParser, parseStringIntengerToBoolean } from './parsers';
...
const birthdateToAge = birthdate => moment().diff(moment(birthdate, CHILD_BIRTHDATE_FORMAT), 'years');
const resolveChildAge = age => isChildBirthdate(age) ? birthdateToAge(age) : age;
export const compileChildrenToPeopleField = children => children.map(resolveChildAge).map(age => String(Math.max(age, 1)).padStart(CHILD_AGE_FIELD_LENGTH, '0')).join('');
...
const DEFAULTS = {
  ...
  [QUERY_PARAMS.TRANSPORTS]: List().push(Map({
    'air': true,
    'bus': true,
    'train': true,
    'ship': true,
...
function compileQuery(query) {
  const fieldsToCompilers = {
    ...
    [QUERY_PARAMS.TRANSPORTS]: transportsList => immutableArrayCompiler(transportsList.map(binaryCompiler)),
...
function convertToOtpQuery(query) {
  const converters = {
    [QUERY_PARAMS.DEPARTURES]: list => {
      ...
      return value !== EMPTY_DEPARTURE_VALUE ? {
        'from': value
      } : {};
    },
...
function parseQueryParam(currentValue, paramName, rawValue) {
  const paramsToParsers = {
    ...
    [QUERY_PARAMS.TRANSPORTS]: (raw, _ref) => {
      ...
      return arrayParser(raw).map(value => binaryParser(value, {
    ...
    [QUERY_PARAMS.CHILDREN]: value => {
      ...
      const isValidBirthdate = item => moment(item, CHILD_BIRTHDATE_FORMAT, true).isValid();
  ...
  const parseQueryParam = (currentValue, paramName, rawValue) => {
    const paramsToParsers = {
      ...
      [QUERY_PARAMS.TRANSPORTS]: (list, _ref2) => {
        ...
        const arrayParser = createImmutableArrayParser(List);
        return arrayParser(list).map(value => binaryParser(value, {
          prevValue: prevList.first()
        }));
      },
      [QUERY_PARAMS.FOOD]: binaryParser,
...
export { SHORT_QUERY_NAMES, QUERY_PARAMS, GLUE, createQuery, createResultBones, compileQuery, compileSearchQuery, compileQueryToHash, convertToOtpQuery, parseQueryString, parseHashToQuery };