...
export const rangeCompiler = value => {
  ...
  return (from === to ? [from] : [from, to]).join(GLUE.range);
};
...
export const datesCompiler = value => {
  ...
  if (isSetRangeDateField) {
    ...
    return [moment(value.get('from')).add(range, 'days').format('D.M.Y'), `${RANGE_DATE_TAG}${range}`].join('');
  }
  return [value.get('from'), value.get('to')].map(date => date ? moment(date).format('D.M.Y') : GLUE.empty).join(GLUE.range);
};
